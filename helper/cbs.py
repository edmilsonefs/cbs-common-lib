...
class CommonHelper(TestlioAutomationTest):
    ...
    def _hide_keyboard(self):
        for x in range(0, 3):
            try:
                local.driver.hide_keyboard()
                sleep(1)
            except:
                pass
        sleep(2)

    def _short_swipe_up(self, duration=1000):
        size = local.driver.get_window_size()
        x = size['width'] / 2
        start_y = size['height'] / 2
        end_y = size['height'] - 50

        local.driver.swipe(x, start_y, x, end_y, duration)
        sleep(1)

    def _short_swipe_down(self, duration=4000):
        size = local.driver.get_window_size()
        x = size['width'] / 2
        start_y = size['height'] / 2
        end_y = 50
